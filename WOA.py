...
def WOA(objf, lb, ub, Dim, pop_size, Max_iter, ProbParam):
    n = ProbParam[0]
    m = ProbParam[1]
    ETC = ProbParam[2]
    VM = ProbParam[3]
    ...
    for i in range(Dim):
        Positions[:, i] = (numpy.random.uniform(0, 1, pop_size) * (ub[i] - lb[i]) + lb[i])
    ...
    while t < Max_iter:
        for i in range(0, pop_size):
            ...
            fitness = objf(Positions[i, :], n, m, ETC, VM, SP, ETime, DTime)
